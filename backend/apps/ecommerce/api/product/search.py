...
class SearchProduct(APIView):
    def get_queryset_filters(self, filters: dict):
        ...
        if not any([query, category, min_price, max_price]):
            return None

        if query:
            Qfilters |= Q(product_name__icontains=query)
            Qfilters |= Q(description__icontains=query)

        if category:
            Qfilters |= Q(category_id=category)

        if min_price:
            Qfilters |= Q(price__gte=min_price)

        if max_price:
            Qfilters |= Q(price__lte=max_price)
    ...
    def get(self, *args, **kwargs):
        ...
        if not filters:
            return Response({"error": "provide search parameters."}, status=400)
        ...
        product_queryset = Product.objects.filter(filters).annotate(
            ...
            # rating=Coalesce(
            #     Subquery(
            #         ProductReview.objects.filter(product=OuterRef("id"))
            #         .values("rating")
