...
export const OrdersDetail: React.FC = () => {
    ...
    return (
        <>
            {/* <Header /> */}
            <div className="flex flex-col  max-w-6xl mx-auto p-4 md:p-6 ">
                <div className='flex gap-3 items-center p-2 mb-4'>
                ...
                <div className='border border-gray-200 rounded-xl p-4 mb-6 shadow-sm'>
                    {data.items.map((item: any, index: number) => (
                        <div key={index} className="bg-gray-50 border border-gray-100 rounded-lg p-3 my-4 flex justify-between items-center">
                            <div className="flex items-center mb-2 sm:mb-0">
                                ...
                                <span className="text-xs md:text-sm text-ellipsis text-wrap">{item.product.name}</span>
                            </div>
                            <div className="flex items-center">
                                <span className="text-xs md:text-sm font-medium"><strong>Qty:</strong> {item.quantity}</span>
                                <span className="ml-3 text-sm font-medium hidden md:block"><strong>$</strong> {item.price}</span>
                            </div>
                        </div>
                    ))}

                    <div className="flex flex-col sm:flex-row justify-between mb-6">
                        <div className="w-full mb-4 sm:mb-0 bg-gray-50 border border-gray-100 rounded-lg p-3 flex flex-col justify-between">
                            <div>
                            ...
                            <div>
                                <Button className='text-xs sm:text-sm md:text-base'>Buy this again</Button>
                            </div>
                        </div>

                        <div className="w-full sm:ml-4 bg-gray-50 border border-gray-100 rounded-lg p-3">
